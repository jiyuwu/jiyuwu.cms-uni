			...
			<view class="uni-select" :style="{height:multiple?'100%':' 35px;'}"
				...
				<view class="uni-select__input-box" :style="{height:multiple?'100%':'35px;'}" @click="toggleSelector">
					<view class="" style="display: flex;flex-wrap: wrap;width: 100%;" v-if="multiple&&current.length>0">
						...
						<input v-if="filterable&&!disabled" class="uni-select__input-text" type="text"
							style="font-size: 12px;height: 52rpx;margin-left: 6px;width: auto;" placeholder="请输入"
							v-model="filterInput">
					</view>
					<view v-else-if="current&&current.length>0&&!filterable" class="uni-select__input-text">{{current}}
					</view>
					<input v-else-if="filterable" :disabled="disabled" class="uni-select__input-text" type="text"
						style="font-size: 12px;" :placeholder="placeholderOld" v-model="filterInput">
